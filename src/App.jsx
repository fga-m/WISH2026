...
import { 
  ...
  Maximize2, Ticket, Map as MapPinSquare, Plus, Minus, RefreshCcw,
  Users, LogOut
} from 'lucide-react';
...
function normalizeString(str) {
  return str ? str.toString().toLowerCase().trim().replace(/\s+/g, ' ').replace(/[^\w\s]/g, '') : '';
}
...
function getDirectDriveLink(url) {
  if (!url || !String(url).includes('drive.google.com')) return url || '';
  const idMatch = String(url).match(/\/d\/([a-zA-Z0-9_-]+)/) || String(url).match(/id=([a-zA-Z0-9_-]+)/);
...
function formatTimestamp(ts) {
  ...
  return !isNaN(date) ? date.toLocaleDateString([], { day: 'numeric', month: 'short' }) + ', ' + date.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }) : ts;
}

// Sub-components
const NavItem = React.memo(({ icon: Icon, label, isActive, onClick, badge = 0 }) => (
  <button onClick={onClick} className={`flex flex-col items-center justify-center w-full h-full space-y-1 transition-all ${isActive ? 'text-[#4563AD]' : 'text-gray-400 hover:text-gray-600'}`}>
    <div className={`p-1.5 rounded-xl transition-all relative ${isActive ? 'bg-[#4563AD]/10' : ''}`}>
      <Icon size={22} />
      {badge > 0 && <span className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[9px] font-black text-white ring-2 ring-[#FCF5EB] animate-pulse">{badge > 9 ? '9+' : badge}</span>}
    </div>
    <span className="text-[10px] font-extrabold uppercase tracking-tight leading-tight">{label}</span>
  </button>
));

const ExpandableText = React.memo(({ text, maxLength = 250, className = "text-gray-600 text-sm leading-relaxed font-medium" }) => {
  const [isExpanded, setIsExpanded] = useState(false);
  const textStr = String(text || '');
  if (textStr.length <= maxLength) return <p className={className}>{textStr}</p>;
  return (
    <div className={className}>
      <span>{isExpanded ? textStr : `${textStr.substring(0, maxLength).trim()}...`}</span>
      <button onClick={(e) => { e.stopPropagation(); setIsExpanded(!isExpanded); }} className="ml-1 text-[#ED4E23] font-bold hover:underline">{isExpanded ? 'See less' : 'See more'}</button>
    </div>
  );
});
...
function WorkshopDetailView({ workshop, onBack }) {
  ...
  const getMapsUrlForRoom = (roomName) => {
    ...
    const clean = roomName.toLowerCase().trim();
    let venue = VENUE_MAP.find(v => v.rooms.some(r => r.name.toLowerCase().trim() === clean));
    if (!venue) venue = VENUE_MAP.find(v => v.zone.toLowerCase().trim() === clean || clean.includes(v.zone.toLowerCase().trim()));
    ...
    <div className="flex flex-col min-h-screen bg-[#FCF5EB] animate-in slide-in-from-right-8 duration-300 pb-24 text-left text-gray-900">
      <div className="w-full max-w-2xl mx-auto px-6">
        <div className="py-4 border-b border-[#E8BA21]/20 flex items-center gap-2 sticky top-0 bg-[#FCF5EB]/90 backdrop-blur-sm z-10">
          <button onClick={onBack} className="p-2 -ml-2 text-[#4563AD] hover:bg-[#4563AD]/10 rounded-full"><ChevronLeft size={24} /></button>
          <span className="font-extrabold text-[#4563AD] text-sm uppercase tracking-wider">Details</span>
        </div>
        <div className="py-8">
          ...
          {workshop.sessions?.length > 0 && (
            <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-6 mb-8">
              <h3 className="text-[#4563AD] font-bold mb-4 text-xs uppercase tracking-widest flex items-center gap-2"><Calendar size={14} /> Schedule</h3>
              <div className="space-y-4">
                {workshop.sessions.map((s, i) => (
                  <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 last:pb-0 border-b border-gray-50 last:border-0">
                    <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-[#FCF5EB] flex items-center justify-center text-[#ED4E23] shrink-0"><Clock size={18} /></div><div><p className="text-sm font-extrabold uppercase">{s.day} Session</p><p className="text-xs font-bold text-gray-400">{s.time}</p></div></div>
                    <a href={getMapsUrlForRoom(s.room)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-gray-50 hover:bg-[#4563AD]/5 text-[#4563AD] px-4 py-2.5 rounded-xl border border-gray-100 transition-all group shrink-0"><MapPin size={14} className="text-[#E8BA21]" /><span className="text-xs font-bold">{s.room}</span><ExternalLink size={10} className="opacity-30" /></a>
                  </div>
                ))}
              </div>
            </div>
          )}
          <div className="prose prose-sm text-gray-600 leading-relaxed bg-white p-6 md:p-8 rounded-[2rem] border border-gray-100 shadow-sm mb-8"><h3 className="text-gray-900 font-bold mb-3 text-lg font-serif">About</h3>{workshop.description ? <ExpandableText text={workshop.description} maxLength={3000} /> : <p className="italic text-gray-400">Loading...</p>}</div>
          {workshop.biography && (
            <div className="bg-white p-6 md:p-8 rounded-[2rem] border border-[#4563AD]/10 shadow-sm relative overflow-hidden">
              <div className="absolute top-0 right-0 w-24 h-24 bg-[#E8BA21]/10 rounded-bl-full"></div>
              <h3 className="text-[#4563AD] font-bold mb-4 text-lg font-serif relative z-10">Speaker</h3>
              <div className="flex items-start gap-4 relative z-10">
                <div className="w-20 h-20 rounded-2xl bg-[#FCF5EB] border-2 border-[#ED4E23] flex items-center justify-center overflow-hidden shrink-0 uppercase font-black text-[#ED4E23]">{workshop.photo ? <img src={getDirectDriveLink(workshop.photo)} alt={workshop.speaker} className="w-full h-full object-cover" /> : workshop.speaker?.charAt(0)}</div>
                <div className="flex-1"><h4 className="font-extrabold text-base leading-tight">{workshop.speaker}</h4><ExpandableText text={workshop.biography} maxLength={250} className="text-sm text-gray-600 mt-1.5 leading-relaxed" /></div>
              </div>
            </div>
          )}
        </div>
...
export default function App() {
  ...
  // Memory Cache for Registry to prevent redundant fetches
  const registryCache = useRef(null);
  ...
  const processUser = useCallback(async (u, emailStr, allPeers = []) => {
    ...
    const userName = `${String(u[fKey] || '')} ${String(u[lKey] || '')}`.trim();
    
    setConferenceUser({ name: userName, email: emailStr, workshops: u });
    if (allPeers.length > 0) setMatchingUsers(allPeers);
    ...
    if (user) {
      try {
        await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'session', 'current'), { 
          email: emailStr, selectedUser: userName, updatedAt: new Date().toISOString(), notifVersion: NOTIFICATION_VERSION 
        }, { merge: true });
  ...
  const fetchRegistry = async () => {
    if (registryCache.current) return registryCache.current;
    const res = await fetch(`${LINKS.itineraries}&t=${Date.now()}`, { cache: "no-store" });
    const data = parseCSV(await res.text());
    registryCache.current = data;
    return data;
  };

  const handleLogin = async (e) => {
    if (e) e.preventDefault();
    if (!email.trim() || isLoadingUser) return;
    setIsLoadingUser(true);
    setError('');
    try {
      const rawData = await fetchRegistry();
      const emailStr = email.trim().toLowerCase();
      const users = rawData.filter(row => Object.values(row).some(val => String(val).toLowerCase().trim() === emailStr));
      if (users.length === 1) await processUser(users[0], emailStr, users);
      else if (users.length > 1) setMatchingUsers(users);
      else setError(`"${emailStr}" not found in registry.`);
      ...
      if (user) {
        try {
          ...
          if (snap.exists()) {
            ...
            if (data.notifVersion === NOTIFICATION_VERSION && data.lastSeenUpdates) setLastSeenUpdates(data.lastSeenUpdates);
            if (data.email) {
               setEmail(data.email);
               const rawData = await fetchRegistry();
               ...
               if (users.length > 0) {
                 ...
                 if (data.selectedUser) {
                    const match = users.find(u => `${String(u['namefirst'] || '')} ${String(u['namelast'] || '')}`.trim() === data.selectedUser);
    ...
    const fetchData = async () => {
      try {
        const [catRes, updRes] = await Promise.all([
          fetch(`${LINKS.workshopCatalog}&t=${Date.now()}`),
          fetch(`${LINKS.updatesFeed}&t=${Date.now()}`)
        ]);
        const catalogData = parseCSV(await catRes.text());
        ...
        const mappedUpdates = parseCSV(await updRes.text()).map(u => ({
          ...
          ms: !isNaN(new Date(u.timestamp).getTime()) ? new Date(u.timestamp).getTime() : 1
        }));
  ...
  const handleLogout = async () => {
    ...
    setIsUserMenuOpen(false);
    const user = auth.currentUser;
    if (user) await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'session', 'current'), { email: null, selectedUser: null }, { merge: true }).catch(()=>{});
  };
  ...
  if (!isDataLoaded || !isSessionRestored) return (
    <div className="min-h-screen bg-[#FCF5EB] flex flex-col items-center justify-center p-10 text-center text-gray-900">
      ...
      <h2 className="text-xl font-extrabold font-serif">WISH 2026</h2>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#FCF5EB] flex flex-col font-sans text-gray-900 selection:bg-[#E8BA21]/30 text-left overflow-hidden">
        ...
        <div className="fixed inset-0 z-[100] bg-[#FCF5EB]/95 backdrop-blur-md flex flex-col animate-in slide-in-from-bottom-8 duration-300">
          <div className="w-full max-w-2xl mx-auto flex flex-col h-full px-6">
            <div className="py-6 flex items-center justify-between border-b border-[#E8BA21]/20">
              <div className="flex flex-col"><span className="text-[10px] font-black text-[#ED4E23] uppercase tracking-widest">Signed in as {conferenceUser?.email}</span><h2 className="text-2xl font-extrabold font-serif">Profiles</h2></div>
              <button onClick={() => setIsUserMenuOpen(false)} className="p-3 bg-white rounded-2xl shadow-sm border border-gray-100 text-gray-400 hover:text-gray-900"><X size={24} /></button>
            </div>
            <div className="flex-1 overflow-y-auto py-8 space-y-4">
              {matchingUsers.map((u, i) => {
                const name = `${String(u['namefirst'] || '')} ${String(u['namelast'] || '')}`.trim();
                const isActive = conferenceUser?.name === name;
                return (
                  <button key={i} onClick={() => processUser(u, conferenceUser.email, matchingUsers)} className={`w-full p-6 rounded-[2rem] flex items-center justify-between transition-all border ${isActive ? 'bg-[#4563AD] border-[#4563AD] text-white shadow-lg' : 'bg-white border-gray-100 text-gray-800 shadow-sm hover:border-[#E8BA21]/40'}`}>
                    <div className="flex items-center gap-4"><div className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-sm ${isActive ? 'bg-white/20' : 'bg-[#FCF5EB] text-[#4563AD]'}`}>{name.charAt(0)}</div><span className="font-extrabold text-lg">{name}</span></div>
                    {isActive ? <CheckCircle2 size={24} /> : <ChevronRight size={20} className="text-gray-300" />}
                  </button>
                );
              })}
            </div>
            <div className="py-8 border-t border-[#E8BA21]/20"><button onClick={handleLogout} className="w-full p-6 bg-red-50 text-red-500 rounded-[2rem] flex items-center justify-center gap-3 font-extrabold text-lg hover:bg-red-100"><LogOut size={20} /> Logout</button></div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-sm flex flex-col items-center justify-center animate-in fade-in duration-200 overflow-hidden touch-none" onWheel={(e) => setZoom(Math.min(Math.max(1, zoom + e.deltaY * -0.005), 5))}>
          <div className="absolute top-0 left-0 right-0 p-6 flex justify-between items-center z-[110] pointer-events-none">
            <div className="bg-black/40 backdrop-blur-md px-4 py-2 rounded-2xl border border-white/10 flex items-center gap-4 pointer-events-auto">
              <button onClick={() => setZoom(Math.max(1, zoom - 0.5))} className="text-white/80 hover:text-white p-1"><Minus size={20}/></button>
              <span className="text-white font-black text-xs min-w-[3rem] text-center">{Math.round(zoom * 100)}%</span>
              <button onClick={() => setZoom(Math.min(5, zoom + 0.5))} className="text-white/80 hover:text-white p-1"><Plus size={20}/></button>
              <div className="w-px h-4 bg-white/10 mx-1"></div>
              <button onClick={() => { setZoom(1); setOffset({x:0,y:0}); }} className="text-white/80 hover:text-white p-1"><RefreshCcw size={16}/></button>
            </div>
            <button onClick={() => setSelectedImage(null)} className="p-3 bg-white/10 hover:bg-white/20 backdrop-blur-md text-white rounded-full pointer-events-auto"><X size={28} /></button>
          </div>
          ...
          <div className="absolute bottom-10 bg-black/40 backdrop-blur-md px-6 py-3 rounded-full border border-white/10 text-white/50 text-[10px] font-bold tracking-widest pointer-events-none">{zoom > 1 ? "Drag to move • Double-tap to reset" : "Pinch or scroll to zoom"}</div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] bg-[#FCF5EB]/95 backdrop-blur-md flex flex-col animate-in slide-in-from-bottom-8 duration-300">
          <div className="w-full max-w-2xl mx-auto flex flex-col h-full px-6 text-left">
            <div className="py-6 flex items-center justify-between border-b border-[#E8BA21]/20">
              ...
              <button onClick={() => setSelectedSlot(null)} className="p-3 bg-white rounded-2xl shadow-sm border border-gray-100 text-gray-400 hover:text-gray-900"><X size={24} /></button>
            </div>
            <div className="flex-1 overflow-y-auto py-8 space-y-4 pb-12">
              {selectedSlot.matches.length === 0 && <div className="text-center py-20 text-gray-400 italic">No workshops found for this slot.</div>}
      ...
      <header className="w-full bg-[#FCF5EB] border-b border-[#E8BA21]/20 sticky top-0 z-40 h-20 shrink-0">
        <div className="max-w-2xl mx-auto p-5 flex justify-between items-center h-full">
          <div className="flex items-center gap-2"><div className="w-10 h-10 bg-[#ED4E23] rounded-xl flex items-center justify-center text-white font-black text-2xl shadow-sm">W</div></div>
          {conferenceUser && (
            <button onClick={() => setIsUserMenuOpen(true)} className="w-10 h-10 bg-white rounded-2xl flex items-center justify-center border border-gray-100 font-black text-sm text-[#4563AD] uppercase shadow-sm active:scale-95 transition-all relative">
              {conferenceUser.name.charAt(0)}
              {matchingUsers.length > 1 && <div className="absolute -bottom-1 -right-1 bg-[#4563AD] text-white p-0.5 rounded-md border-2 border-[#FCF5EB]"><Users size={10} /></div>}
            </button>
          )}
        </div>
      </header>
          ...
          <>
            {activeTab === 'updates' && (
              <div className="space-y-8 animate-in fade-in text-left text-gray-900">
                ...
                <div className="space-y-4">
                  {updates.length === 0 && <div className="text-center py-20 text-gray-400 italic">Checking for news...</div>}
                    ...
                    <div key={idx} className="bg-white rounded-[1.5rem] border border-gray-100 shadow-sm overflow-hidden flex items-start gap-4 p-5 animate-in slide-in-from-bottom-4">
                      ...
                      {post.image && (
                        <div className="w-24 h-24 sm:w-28 sm:h-28 shrink-0 rounded-2xl overflow-hidden bg-gray-50 border border-gray-100 cursor-pointer relative group" onClick={() => setSelectedImage(post.image)}>
                          <img src={getDirectDriveLink(post.image)} className="w-full h-full object-cover group-hover:scale-105 transition-all" alt="Post" onError={(e) => e.target.style.display = 'none'} />
                          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 flex items-center justify-center transition-colors"><Maximize2 size={20} className="text-white opacity-0 group-hover:opacity-100" /></div>
                        </div>
                      )}
                    </div>
                  ))}
                ...
                <div className="grid gap-4">
                  {filteredWorkshops.map(w => (
                    <div key={w.id} onClick={() => setSelectedWorkshopId(w.id)} className="bg-white p-6 rounded-[2rem] shadow-sm border border-transparent hover:border-[#E8BA21]/30 cursor-pointer transition-all flex flex-col group">
                      ...
                      {w.sessions?.length > 0 && <div className="mt-4 flex flex-col gap-2 border-t border-gray-50 pt-4">{w.sessions.map((s, i) => (<div key={i} className="flex items-center gap-3 bg-gray-50/80 px-3 py-1.5 rounded-xl border border-gray-100/50 w-fit"><div className="flex items-center gap-1 text-[9px] font-bold text-gray-500 uppercase tracking-tighter"><Clock size={10} className="text-gray-400" />{s.day} {s.time}</div><div className="w-px h-2 bg-gray-200" /><div className="flex items-center gap-1 text-[9px] font-bold text-[#4563AD] uppercase tracking-tighter"><MapPin size={10} className="opacity-50" />{s.room}</div></div>))}</div>}
                    </div>
                  ))}
                </div>
              </div>
              ...
              conferenceUser ? (
                <div className="space-y-8 text-left animate-in fade-in text-gray-900">
                  <div className="flex justify-between items-end">
                    <div><h2 className="text-4xl font-extrabold text-[#ED4E23] font-serif">My WISH</h2><p className="text-xs text-gray-500 font-bold uppercase">Itinerary</p></div>
                    {matchingUsers.length > 1 && (<button onClick={() => setIsUserMenuOpen(true)} className="text-[10px] font-bold text-[#4563AD] bg-[#4563AD]/5 px-3 py-1.5 rounded-lg border border-[#4563AD]/20 flex items-center gap-2 uppercase tracking-widest"><Users size={12} /> Switch</button>)}
                  </div>
                  <DaySelector selectedDay={selectedDay} onDayChange={setSelectedDay} />
                  <div className="space-y-6">
                    {MASTER_SCHEDULE.find(d => d.date === selectedDay)?.events.map(ev => {
                      const sel = String(conferenceUser.workshops[ev.id.toLowerCase()] || '');
                      const ws = workshopLookupMap.get(sel.toLowerCase().trim()) || Array.from(workshopLookupMap.values()).find(w => normalizeString(w.title) === normalizeString(sel));
                      const isL = sel.toLowerCase().includes('lunch') || ws?.id === 'lunch-special';
                      const match = ws?.sessions?.find(s => s.day === selectedDay.substring(0, 3) && s.time === ev.time);
                      return (
                        <div key={ev.id} className="flex gap-4">
                          <div className="w-16 text-right font-bold text-gray-400 text-sm py-4">{ev.time}</div>
                          <div className={`flex-1 p-5 rounded-3xl border transition-all relative overflow-hidden ${(ev.type === 'main' || ws) ? 'bg-white border-[#4563AD] shadow-md' : 'bg-white/50 border-gray-100 shadow-sm'}`} onClick={() => (ws && !isL) ? setSelectedWorkshopId(ws.id) : null}>
                            {(ev.type === 'main' || ws) && <div className={`absolute top-0 left-0 w-1.5 h-full ${ev.type === 'main' ? 'bg-[#4563AD]' : 'bg-[#E8BA21]'}`} />}
                            <h4 className={`font-bold text-lg ${!ws && ev.type === 'workshop_slot' ? 'italic text-gray-400' : ''}`}>{ws ? ws.title : (ev.type === 'workshop_slot' ? (sel || 'No session selected') : ev.title)}</h4>
                            {ws && !isL && <p className="text-xs text-[#ED4E23] font-bold mt-1 uppercase">with {ws.speaker}</p>}
                            {match && <div className="mt-3 flex items-center gap-1.5 text-[10px] font-bold uppercase text-gray-400 tracking-wider"><MapPin size={12} className={ev.type === 'main' ? "text-[#4563AD]/40" : "text-[#E8BA21]/40"} />{match.room}</div>}
                          </div>
                        </div>
                      );
                    })}
                ...
                <div className="flex flex-col space-y-8 text-left animate-in fade-in">
                  {matchingUsers.length > 0 ? (
                    ...
                    <>
                      ...
                      <div className="bg-white p-8 md:p-10 rounded-[3rem] border border-gray-100 shadow-xl shadow-[#4563AD]/5 mb-4 text-left"><h2 className="text-2xl font-extrabold mb-2">Sign In</h2><p className="text-sm text-gray-400 font-medium mb-8">Enter your registered email.</p><form onSubmit={handleLogin} className="space-y-4"><input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Email address" className="w-full p-5 rounded-2xl border border-gray-100 focus:ring-4 focus:ring-[#E8BA21]/10 outline-none text-gray-900 font-medium transition-all" required />{error && <div className="text-red-500 text-xs font-bold bg-red-50 p-4 rounded-xl flex items-center gap-2 animate-bounce"><AlertCircle size={16}/> {error}</div>}<button type="submit" disabled={isLoadingUser} className="w-full bg-[#ED4E23] text-white font-extrabold py-5 rounded-2xl shadow-lg flex items-center justify-center gap-2 text-lg hover:bg-[#ED4E23]/90 transition-all active:scale-95">{isLoadingUser ? "Checking..." : "Access Schedule"}</button></form></div>
                      <div className="bg-[#4563AD]/5 p-8 md:p-10 rounded-[3rem] border border-[#4563AD]/20 text-center overflow-hidden relative group"><Ticket className="mx-auto text-[#4563AD] mb-4 group-hover:scale-110 transition-all" size={40} /><h2 className="text-2xl font-extrabold text-gray-900 mb-2 font-serif">Need a Ticket?</h2><p className="text-sm text-gray-500 font-medium mb-8">Purchase through Brushfire.</p><button onClick={openBrushfire} className="w-full bg-[#4563AD] text-white font-extrabold py-5 rounded-2xl shadow-lg hover:bg-[#4563AD]/90 transition-all active:scale-95">Get Tickets</button></div>
            ...
            {activeTab === 'map' && (
              <div className="animate-in fade-in space-y-10 text-left text-gray-900">
                ...
                <div onClick={() => setSelectedImage(LINKS.siteMapImage)} className="bg-white rounded-[3rem] border border-[#4563AD]/20 shadow-sm overflow-hidden cursor-pointer group relative transition-all active:scale-95"><div className="aspect-[16/9] w-full bg-gray-100 overflow-hidden relative"><img src={getDirectDriveLink(LINKS.siteMapImage)} className="w-full h-full object-cover group-hover:scale-105 transition-all" alt="Map" /><div className="absolute inset-0 bg-black/20 group-hover:bg-black/10 transition-colors flex items-center justify-center"><div className="bg-white/90 backdrop-blur p-4 rounded-2xl shadow-xl flex items-center gap-3 animate-in zoom-in"><MapPinSquare className="text-[#4563AD]" size={24} /><span className="font-extrabold text-[#4563AD] text-sm uppercase tracking-widest">Lexton Road Map</span><Maximize2 size={16} className="text-gray-400" /></div></div></div></div>
                <div className="space-y-8">
                  {VENUE_MAP.map((location, idx) => (
                    <div key={idx} className="bg-white rounded-[3rem] border border-gray-100 shadow-sm overflow-hidden animate-in slide-in-from-bottom-4" style={{animationDelay: `${idx*100}ms`}}>
                      <div className="p-8 flex items-start gap-5 border-b border-gray-50 bg-gray-50/40"><div className="w-12 h-12 rounded-2xl bg-[#4563AD]/10 flex items-center justify-center text-[#4563AD] shrink-0"><location.icon size={22} /></div><div className="flex-1"><h3 className="text-xl font-extrabold">{location.zone}</h3><a href={location.mapUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-xs text-[#E8BA21] font-bold mt-1 hover:text-[#4563AD] transition-all">{location.address}<ExternalLink size={12} className="text-gray-300" /></a>{location.levelMaps && (<div className="mt-4 flex flex-wrap gap-2">{location.levelMaps.map((lvl, lIdx) => (<button key={lIdx} onClick={(e) => { e.preventDefault(); e.stopPropagation(); setSelectedImage(lvl.url); }} className="bg-white border border-[#4563AD]/20 hover:border-[#4563AD] text-[#4563AD] text-[10px] font-black uppercase px-3 py-1.5 rounded-xl shadow-sm flex items-center gap-1.5"><Maximize2 size={12} /> {lvl.label}</button>))}</div>)}</div></div>
                      <div className="p-6 md:p-8 grid grid-cols-1 sm:grid-cols-2 gap-3">{location.rooms.map((room, rIdx) => (<div key={rIdx} className="flex items-center justify-between p-4 bg-white rounded-2xl border border-gray-100"><span className="text-sm font-bold text-gray-700">{room.name}</span><span className="text-[10px] uppercase font-extrabold text-gray-400 bg-gray-50 px-2 py-0.5 rounded-lg border border-gray-100">{room.note}</span></div>))}</div>
                    </div>
                  ))}
                </div>
              </div>
